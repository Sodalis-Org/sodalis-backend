...
router.post('/',
    ...
    async (req, res) => {
        ...
        // Récupérer le nom du créateur pour enrichir le payload de l'événement
        ...
        if (priority === 'URGENT') {
            try {
                ...
                console.error('❌ Erreur gRPC CreateTask (URGENT) :', err.message);
            }
        }
...
router.patch('/:id/status',
    ...
    async (req, res) => {
        ...
        if (req.user.role !== 'ADMIN' && req.user.coloc_id !== ticket.coloc_id) {
            return res.status(403).json({ error: 'Non autorisé — Vous n\'appartenez pas à cette colocation' });
        }
...
// PATCH /maintenance/:id/assign — Assigner un ticket (ADMIN uniquement)
router.patch('/:id/assign',
    ...
    async (req, res) => {
        ...
        // Vérifie que l'assigné appartient bien à la coloc du ticket
